...
def fit_with_KerasNN(setup, submodel_no, X, y, tol, slowdown_factor, early_stop_trials):


    filename = "NN_{}.h5".format(submodel_no)
    log_filename = "NN_fit_{}_log.log".format(submodel_no)
...
def read_data_from_one_dir(syst, setup, num_systems):
    temp_cwd = os.getcwd()
    os.chdir('./'+syst)
    ...
    num_random_per_molecule = int(math.ceil(float(setup["random_pick"])/float(num_systems)))
    print "Number of random samples: {}".format(num_random_per_molecule)
    ...
    try:
        molecule_random_data = random_subsampling(pickle.load(open(random_filename,'rb')),num_random_per_molecule)
...
if __name__ == "__main__":
    ...
    data_dir_name = setup["working_dir"] + "/data/"
    systems = os.listdir(data_dir_name)
    num_systems = len(systems)

    for s in systems:

        os.chdir(data_dir_name)
        X_test,y, dens = read_data_from_one_dir(s, setup, num_systems)
        print "done collecting data, in main now"
   ...
   # hf.create_dataset('y', data=np.asarray(y))
        ...
        os.chdir(model_save_dir)
        print "in model save dir "+model_save_dir
        ...
        dat = pd.DataFrame()
        dat["density"] = pd.Series(np.asarray(dens)[:,0])
        dat["y"] = pd.Series(np.asarray(y)[:,0])
        dat["LDA_predict_y"] = pd.Series(LDA_predict_y[:,0])
        ...
        models = np.arange(int(no_submodels))+1

        for submodel_no in models:
            ...
            os.chdir(sub_dir)
            print " in subdir"
            NN_model =  keras.models.load_model( "NN_{}.h5".format(submodel_no))
            print "NN_{} loaded".format(submodel_no)
            #save_resulting_figure(dens,result.x,X_train,NN_model,y)

            NN_predict_y = NN_model.predict(X_test)
            predict_y = LDA_predict_y + NN_predict_y
            error = np.absolute(y - predict_y)

            pred_fname = "predictions_submodel_{}.h5".format(submodel_no)
            hf = h5py.File(pred_fname, 'w')
            
            
            dat["NN_predict_y_{}".format(submodel_no)] = pd.Series(NN_predict_y[:,0])
            dat["predict_y_{}".format(submodel_no)] = pd.Series(predict_y[:,0])
            dat["error_{}".format(submodel_no)] = pd.Series(error[:,0])
        
        os.chdir(model_save_dir)
        print "Data saved in data frame"
    
        
        def gen_headers(s): return list(map(lambda x: s+'_{}'.format(x), models))
        
        NN_header_list = gen_headers("NN_predict_y")
        predict_header_list = gen_headers("predict_y")
        error_header_list = gen_headers("error")
        
        residuals = dat[NN_header_list]
        mu_res = residuals.mean(axis=1)
        std_res = residuals.std(axis=1)
        
        errors = dat[error_header_list]
        mu_err = errors.mean(axis=1)
        
        dat['MAE'] = pd.Series(mu_err)
        dat['mu_res'] = pd.Series(mu_res)
        dat['std_res'] = pd.Series(std_res)
        dat.to_hdf('predicted_data_systemwise.h5',s)
    
    # plt.errorbar(dens, mu, sig, fmt='o')
    # plt.title('Uncertainty (std dev) with each residual point')
    # plt.xlabel('density')
    # plt.ylabel('residuals')
    # #plt.xlabel('$\rho$')
    # plt.savefig('res_uncertainty.png')
    
    
    
    # plt.plot(dens, mu_err,'go')   
    # plt.title('MAE in predicted data')
    # #plt.xlabel('$\rho$')
    # plt.show()
    # print 'Saved as data frame'
