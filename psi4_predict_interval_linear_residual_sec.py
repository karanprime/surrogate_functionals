...
def load_data_each_block(molecule, functional, i, j, k, target, gamma, num_desc_deri, num_desc_deri_squa,
                         num_desc_ave_dens, desc_transform='real', target_transform='real'):
...
def predict_each_block(model_dict, X, y):
    ...
    for key in model_dict:
        ...
        temp = model_dict[key]['li_model'].predict(temp_dens.reshape(len(temp_dens), 1)) + model_dict[key][
            'model'].predict(model_dict[key]['X'])
...
def process_one_molecule(molecule, model_dict, functional, h, N, log_filename, target, gamma, num_desc_deri,
    ...
    for i, j, k in paramlist:
        sum_error, y_predict_sum, y_sum = process_each_block(molecule, model_dict, functional, i, j, k, h, N,
                                                             log_filename, target, gamma, num_desc_deri,
                                                             num_desc_deri_squa, num_desc_ave_dens)
...
def process_each_block(molecule, model_dict, functional, i, j, k, h, N, log_filename, target, gamma, num_desc_deri,
                       num_desc_deri_squa, num_desc_ave_dens):
    log(log_filename, "\n\nstart loading: " + molecule + "\t" + i + "\t" + j + "\t" + k)
    start = time.time()
    X, y = load_data_each_block(molecule, functional, i, j, k, target, gamma, num_desc_deri, num_desc_deri_squa,
                                num_desc_ave_dens)
    ...
    log(log_filename,
        "\nenergy: " + str(y_sum) + "\tpredicted energy: " + str(y_predict_sum) + "\tpredicted error: " + str(
            sum_error))
...
def read_model_inputfile(input_filename, h, L, N):
    ...
    with open(input_filename, 'r') as f:
        for line in f:
            ...
            cwd = os.getcwd()
            result_dir = "{}_{}_{}_{}_{}_gamma{}_dev{}_devsq{}_inte{}_{}_{}_models".format(functional,
                                                                                           str(L).replace('.', '-'),
                                                                                           str(h).replace('.', '-'), N,
                                                                                           target, gamma, num_desc_deri,
                                                                                           num_desc_deri_squa,
                                                                                           num_desc_ave_dens,
                                                                                           temp_info[10], temp_info[11])
...
for molecule in molecule_names:
    try:
        temp_error, temp_y_predict, temp_y = process_one_molecule(molecule, model_dict, functional, h, N, log_filename,
                                                                  target, gamma, num_desc_deri, num_desc_deri_squa,
                                                                  num_desc_ave_dens)
...
log(log_filename, "\n\naverage abs error: " + str(np.mean(np.abs(error_list))) + "\tstddev abs error: " + str(
    np.std(np.abs(error_list))))
